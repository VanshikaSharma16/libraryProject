    ...
    <div class="tbl">
        ...
        <table>
            ...
            if (isset($_GET['page'])) {
                ...
                $qry = "select * from department limit $lm offset $pg;";
                $count = $pg + 1;
            } else {
                $qry = "select * from department limit $lm offset $ofst;";
                $count = 1;
            }
            $res = $con->query($qry);
            $id;
            while ($result = $res->fetch_assoc()) {
                ...
                echo "<td><a href='updateDepartment.php?code=$id' style='color:green;'><span class='material-icons-sharp'>
                edit
                </span></a></td>";
                echo "<td><a href='deleteDepartment.php?code=$id' style='color:red;''><span class='material-icons-sharp'>
                delete
                </span></a></td>";
                echo "</tr>";
                $count++;
        ...
        <div class="pagination">
            ...
            $prev = $prev - 1;
            if ($prev > 0) {
                echo "<a href='viewDepartment.php?page=$prev'>&laquo;</a>";
            } else {
                echo "<a href='#'>&laquo;</a>";
            }
            $qryy = "select * from department;";
            $re = $con->query($qryy);
            $rows = mysqli_num_rows($re);
            $box = ceil($rows / $lm);

            for ($i = 1; $i <= $box; $i++) {
                echo "<a href='viewDepartment.php?page=$i'>$i</a>";
            }
            $next = $_GET['page'];
            $next = $next + 1;

            if ($next <= $box) {
                echo "<a href='viewDepartment.php?page=$next'>&raquo;</a>";
            } else {
                echo "<a href='#'>&raquo;</a>";
            }
            ?>
