...
<head>
    ...
    <title>View Accounting Records</title>

    <!-- Temp file css -->
    <link rel="stylesheet" href="../dashboard/tempFinal.css">

    <!-- This file css -->
    <link rel="stylesheet" href="viewAccRcrd.css">
...
<body>
    ...
    <div class="tbl">
        ...
        <div class="cont1">
            <table id="issuedBooksTable">
                ...
                if (isset($_GET['page'])) {
                    ...
                    $qry = "select * from febAcc limit $lm offset $pg;";
                    $count = $pg + 1;
                } else {
                    $qry = "select * from febAcc limit $lm offset $ofst;";
                    $count = 1;
                }
                $res = $con->query($qry);
                $id;
                while ($result = $res->fetch_assoc()) {
                    ...
                    // echo "<td><a href='updateIssueBooks.php?code=$id&bkcode=$bookcode' style='background-color:green; color: var(--color-white); padding: 5.5%; border-radius: .5vw;'>Edit</a></td>";

                    echo "<td><a href='deleteAcc.php?code=$id&stu=$sid' style='background-color:red; color: var(--color-white); padding: 5%; border-radius: .5vw'>Delete</a></td>";
                    echo "</tr>";
                    $count++;
                }
                ?>
        ...
        <div class="pagination">
            ...
            $prev = $prev - 1;
            if ($prev > 0) {
                echo "<a href='viewAccRcrd.php?page=$prev'>&laquo;</a>";
            } else {
                echo "<a href='#'>&laquo;</a>";
            }
            $qryy = "select * from febAcc;";
            $re = $con->query($qryy);
            $rows = mysqli_num_rows($re);
            $box = ceil($rows / $lm);

            for ($i = 1; $i <= $box; $i++) {
                echo "<a href='viewAccRcrd.php?page=$i'>$i</a>";
            }
            $next = $_GET['page'];
            $next = $next + 1;

            if ($next <= $box) {
                echo "<a href='viewAccRcrd.php?page=$next'>&raquo;</a>";
            } else {
                echo "<a href='#'>&raquo;</a>";
            }
            ?>
